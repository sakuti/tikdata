...
export async function computeAuthorStats(videos) {
    ...
    const least = arr.reduce((min, a) => (!min || a.stats.followerCount < min.stats.followerCount) ? a : min, null);
    const most = arr.reduce((max, a) => (!max || a.stats.followerCount > max.stats.followerCount) ? a : max, null);
